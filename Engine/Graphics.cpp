...
void Graphics::DrawThickLine(Vec2D& v0, Vec2D& v1, int thicknes, Color c)
{
	int delta_x = ((int)v1.get_x() - (int)v0.get_x());
	int delta_y = ((int)v1.get_y() - (int)v0.get_y());
	...
	if (abs(sloap) < 1.0f)
	{
		...
		while (x_pos != (int)v1.get_x())
		{
				...
				y_pos < int(Graphics::ScreenHeight))
			{
				...
				for (int j = y_pos - thicknes / 2; j < y_pos + thicknes / 2; j++)
				{
					if (j >= 0 &&
						j < int(Graphics::ScreenHeight))
					{
						PutPixel(x_pos, j, c);
					}
				}
	...
	else
	{
		...
		while (y_pos != (int)v1.get_y())
		{
				...
				y_pos < int(Graphics::ScreenHeight))
			{
				...
				for (int j = x_pos - thicknes / 2; j < x_pos + thicknes / 2; j++)
				{
					if (j >= 0 &&
						j < int(Graphics::ScreenWidth))
					{
						PutPixel(j, y_pos, c);
					}
				}
...
void Graphics::DrawThickPolyline(std::vector<Vec2D>& verts,int thicknes, Color c)
{
	for (auto i = verts.begin(); i != std::prev(verts.end()); i++)
	{
		DrawThickLine(*i, *std::next(i),thicknes, c);
	}
	DrawThickLine(verts.front(), verts.back(), thicknes, c);
}
...
void Graphics::DrawOpenPolyline(std::vector<Vec2D>& verts)
{
	for (auto i = verts.begin(); i != std::prev(verts.end()); i++)
	{
		if ((i->get_x() >= 0 &&
		...
		{
			unsigned char r = static_cast<unsigned char>(i->get_x() / i->get_y()) % 255;
			unsigned char g = static_cast<unsigned char>(i->get_y()) % 255;
			unsigned char b = static_cast<unsigned char>(i->get_x() / i->get_y()) % 255;

			DrawLine(*i, *std::next(i), {r,g,255});
		}
	}
	//DrawLine(verts.front(), verts.back(), c);
}
